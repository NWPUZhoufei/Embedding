...
if __name__ == '__main__':
    ...
    model_E = resnet12(keep_prob=1.0, avg_pool=True, num_classes=64)
    #pretrain_checkpoint = torch.load("C:/Users/zhoufei/Desktop/rfs-master/pretrain/mini_distilled.pth")
    #model_E.load_state_dict(pretrain_checkpoint['model'])
    
    data = torch.randn(2, 3, 224, 224)
    model_E = model_E.cuda()
    data = data.cuda()
    feat, logit = model_E(data, is_feat=True)
    print(feat.shape)
    print(logit.shape)
